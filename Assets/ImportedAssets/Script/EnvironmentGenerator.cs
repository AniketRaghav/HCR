...
public class EnvironmentGenerator : MonoBehaviour
{
    ...
    [SerializeField] private float _stepDistance = 10;
    ...
    {
        GameManager.instance.SwapTerrain(terrainIndex);
    } 
    
    public void GenerateTerrain(Vector3 startPos)
    {
        _spriteShapeController.spline.Clear();
        for (int i = 0; i < _levelLength; i++)
        {
            _lastPos = startPos + new Vector3(i * _xMultiplier, Mathf.PerlinNoise(0, i * _noiseStep) * _yMultiplier);
        ...
        _spriteShapeController.spline.InsertPointAt(_levelLength, new Vector3(_lastPos.x, startPos.y - _bottom));
        _spriteShapeController.spline.InsertPointAt(_levelLength + 1, new Vector3(startPos.x, startPos.y - _bottom));
    }
}
